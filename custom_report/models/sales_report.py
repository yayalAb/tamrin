...
class SalesReportSummary(models.Model):
    ...
    _auto = False
    _order = "sale_date desc, part_number"
    ...
    unit_price = fields.Float(
        string="Unit Price", readonly=True, digits="Product Unit of Measure")
    unit_cost = fields.Float(
        string="Unit Cost", readonly=True, digits="Product Unit of Measure")
    ...
    def init(self):
        ...
        self.env.cr.execute("""
            CREATE OR REPLACE VIEW sales_report_summary AS (
                SELECT 
                    ROW_NUMBER() OVER (ORDER BY sol.product_id) AS id,
                    ROW_NUMBER() OVER (ORDER BY sol.product_id) AS sequence,
                    ...
                    AVG(COALESCE(sol.price_unit, 0.0)) AS unit_price,
                    0.0 AS unit_cost,
                    MIN(so.id) AS order_id,
